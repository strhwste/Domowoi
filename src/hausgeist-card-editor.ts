...
interface SelectorValueDetail<T = unknown> {
  value?: T;
}
...
interface SelectorControlConfig {
  selector: any;
  value: unknown;
  valueType: ValueType;
  disabled?: boolean;
...
const SENSOR_DESCRIPTORS: Record<SensorType, SensorDescriptor> = {
  temperature: {
...
export class HausgeistCardEditor extends LitElement {
  @property({ type: Object }) public config: HausgeistCardConfig = {};
  @state() private _areas: AreaRow[] = [];
  ...
  setConfig(config: HausgeistCardConfig) {
    this.config = {
      debug: false,
      notify: false,
      highThreshold: 2000,
      ...
      <section class="section">
        ...
        ${this._renderField({
          ...
          control: this._renderSelectorControl({
            ...
            onChange: (value) => this._handleConfigValueChange('weather_entity', value as string),
          }),
        })}
        ${this._renderField({
          ...
          control: this._renderSelectorControl({
            ...
            onChange: (value) => this._handleConfigValueChange('default_target_entity', value as string),
          }),
        })}
        ${this._renderField({
  ...
  private _renderArea(area: AreaRow): TemplateResult {
        ...
        <summary>
          ...
          <label class="area-toggle">
            <input
              type="checkbox"
              .checked=${enabled}
              @change=${(event: Event) => this._onAreaToggle(area.area_id, (event.target as HTMLInputElement).checked)}
            />
  ...
  private _renderSensorRow(area: AreaRow, type: SensorType): TemplateResult {
      ...
      <div class="sensor-row ${disabled ? 'is-disabled' : ''}">
        ...
        <div class="sensor-control">
          ${this._renderSelectorControl({
            ...
            onChange: (value) => this._onAreaSensorChange(area.area_id, type, value as string),
          })}
          ${overrideValue
      ...
      <section class="section">
        ...
        ${this._renderField({
          ...
          control: this._renderSelectorControl({
            ...
            valueType: 'string',
            multiline: true,
            onChange: (value) => this._handleConfigValueChange('rulesJson', value as string),
          }),
        })}
      </section>
    `;
  ...
  private _renderMissingSensorsSection(): TemplateResult {
  ...
  private _renderField(field: FieldConfig): TemplateResult {
    return html`
      <div class="field">
  ...
  private _renderSelectorControl(config: SelectorControlConfig): TemplateResult {
    if (this._selectorReady && customElements.get('ha-selector')) {
      return html`
        <ha-selector
          ...
          @value-changed=${(event: CustomEvent<SelectorValueDetail>) => {
            const raw = (event.detail as SelectorValueDetail)?.value ?? (event.detail as any);
      ...
      return html`
        <label class="area-toggle">
          <input
            ...
            ?disabled=${config.disabled ?? false}
            @change=${(event: Event) => config.onChange((event.target as HTMLInputElement).checked)}
          />
          ${config.inlineLabel ?? ''}
        </label>
      `;
    ...
    if (config.valueType === 'number') {
      return html`
        <input
          ...
          @change=${(event: Event) => config.onChange(this._normalizeSelectorValue((event.target as HTMLInputElement).value, 'number'))}
        />
      `;
    }
    ...
    if (config.multiline) {
      return html`
        <textarea
          ...
          ?disabled=${config.disabled ?? false}
          @change=${(event: Event) => config.onChange((event.target as HTMLTextAreaElement).value)}
        ></textarea>
      `;
    }
    ...
    return html`
      <input
        ...
        ?disabled=${config.disabled ?? false}
        @change=${(event: Event) => config.onChange((event.target as HTMLInputElement).value)}
      />
    `;
  }

  private _normalizeSelectorValue(value: unknown, valueType: ValueType) {
  ...
  private _handleConfigValueChange(key: keyof HausgeistCardConfig, value: any) {
    const patch: Partial<HausgeistCardConfig> = { [key]: value } as Partial<HausgeistCardConfig>;
    this._emitConfig(patch);
  }

  private _onAreaToggle(areaId: string, enabled: boolean) {
    this._areas = this._areas.map((area) => (area.area_id === areaId ? { ...area, enabled } : area));
    this._emitConfig();
  }

  private _onAreaSensorChange(areaId: string, type: SensorType, value: string) {
    const overrides = { ...(this.config.overrides ?? {}) };
  ...
  private _emitConfig(patch: Partial<HausgeistCardConfig> = {}) {
    const nextConfig: HausgeistCardConfig = {
      ...this.config,
      ...patch,
    };
    ...
    if (!nextConfig.weather_entity) {
      delete (nextConfig as any).weather_entity;
    }
    if (!nextConfig.default_target_entity) {
      delete (nextConfig as any).default_target_entity;
    }
    if (!nextConfig.rulesJson) {
      delete (nextConfig as any).rulesJson;
    }
  ...
  private _buildAutoMapping(areas: AreaConfig[] = []): Record<string, Record<string, string>> {
  ...
  private _autodetect(areaId: string, type: SensorType): string | undefined {
  ...
  private _matchingEntities(areaId: string, type: SensorType): HassEntity[] {
    if (!this._hass) {
      return [];
    }
    ...
    const states = Object.values(this._hass.states ?? {}) as HassEntity[];
    const candidates = states.filter((state) => this._matchesArea(state, areaId));

    const matches = new Map<string, HassEntity>();
  ...
  private _keywordsForType(type: SensorType): string[] {
    const descriptor = SENSOR_DESCRIPTORS[type];
    const list = [
  ...
  private _domainMatches(entity: HassEntity, domains?: string[]): boolean {
  ...
  private _selectorForSensor(type: SensorType) {
    const descriptor = SENSOR_DESCRIPTORS[type];
    const selector: any = { entity: {} };
  ...
  private _matchesArea(entity: HassEntity, areaId: string): boolean {
    if (!areaId) {
      return false;
    }
  ...
  private _computeMissingSensors(): Array<{ area: AreaRow; sensors: SensorType[] }> {
    return this._areas
  ...
  private _collectAreasFromHass(): AreaRow[] {
    if (!this._hass) {
      return [];
    }
